...
public class RemoteMsgAdminController {
    ...
    public void addWhiteList(String ip, HttpServletResponse response) throws Exception {
        if (!StringUtils.isBlank(ip)) {
            ...
            HttpResponseUtil.write(response, ResponseCode.SUCCESS);
            return;
        } else {
            HttpResponseUtil.write(response, ResponseCode.PARAM_NOT_FORMAT);
        }
    }

    @RequestMapping("/pushMsg")
    public void pushMsg(String contents, String topic, HttpServletResponse response) throws Exception {
        if (!StringUtils.isBlank(contents) && !StringUtils.isBlank(topic)) {
            ...
            if (remoteMsgService.insert(remoteMsg) > 0) {
                ...
                HttpResponseUtil.write(response, ResponseCode.FAIL);
            }
        } else {
            HttpResponseUtil.write(response, ResponseCode.PARAM_NOT_FORMAT);
        }
        ...
        if (id != null) {
            ...
            if (remoteMsg != null) {
                if (remoteMsg.getStatus() != Constants.MSG_OVER) {
                    ...
                    HttpResponseUtil.write(response, ResponseCode.MSG_OVER);
                }
            } else {
                HttpResponseUtil.write(response, ResponseCode.ID_WRONG);
            }
        } else {
            HttpResponseUtil.write(response, ResponseCode.PARAM_NOT_FORMAT);
        }
